...
const PRECACHE_URLS = [
  ...
  '/vite.svg',            // Dev / placeholder icon (if present)
  '/src/assets/main.css'  // Primary stylesheet (dev) — build will serve hashed CSS in production
]
    ...
    (async () => {
      ...
      try {
        ...
        for (const url of PRECACHE_URLS) {
          try { await cache.add(url) } catch (e) { debug('Failed to cache', url, e) }
        }
      }
    ...
    event.respondWith(
      (async () => {
        try {
          ...
          return new Response('<h1>Offline</h1><p>The application is offline and not cached.</p>', {
